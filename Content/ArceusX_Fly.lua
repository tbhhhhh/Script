local Script_Title = "Arceus X <font color=\"rgb(255, 0, 0)\">|</font> 飞行"
...
local toggleBtn
toggleBtn = addToggle("开关飞行: " .. speeds, function(enabled)
	local char = speaker.Character
...
local incraseBtn
incraseBtn = addButton("飞行速度+1", function()
	local char = speaker.Character
	if char and char.Humanoid then
		...
		toggleBtn:WaitForChild("Name").Text = "开关飞行: " .. speeds
	end
end)

local incraseBtnfive
incraseBtnfive = addButton("飞行速度+5", function()
	local char = speaker.Character
	if char and char.Humanoid then
		local hum = char.Humanoid
...
local decraseBtn
decraseBtn = addButton("飞行速度-1", function()
	local char = speaker.Character
	if char and char.Humanoid then
		local hum = char.Humanoid
...
local decraseBtnfive
decraseBtnfive = addButton("飞行速度-5", function()
	local char = speaker.Character
	if char and char.Humanoid then
		local hum = char.Humanoid
